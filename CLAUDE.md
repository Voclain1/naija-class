...
- **Time of day** (a wall-clock time with no date: a bell period's start/end) → Prisma `Int`, PostgreSQL `INTEGER`, **minutes since midnight in the school's wall-clock time** (`490` = 08:10), with a `CHECK (0 <= start < end <= 1440)`. **Not** `@db.Time`: Prisma maps `TIME` to a JavaScript `Date` pinned to 1970-01-01, which serialises with a `Z` and reintroduces the "midnight in which zone?" trap the `DATE` rule above exists to avoid. Integer minutes are zone-free, ordered and checkable in SQL; format once, at display (`formatMinuteOfDay` in `@school-kit/types`). Weekdays alongside them are ISO (1 = Monday … 7 = Sunday) and are derived from a date only via `isoWeekday`. Convention established in Phase 8 / CP3 (2026-09-14, `docs/modules/phase-8.md` §17 D27).
...
| `auth_resolve_guardian_invitation_by_token_hash(token_hash)` | `20260716000000_phase_4_slice_2_guardian_auth`; `WHERE` clause extended `20260916120000_guardian_invitation_revoke` | Public guardian portal invitation-accept endpoints resolve a token hash to `{ invitation_id, school_id, guardian_id, first_name, last_name, email, invited_by, expires_at, accepted_at }` before `withTenant()` can apply. Since 2026-09-16 a **revoked** invitation (`revoked_at IS NOT NULL`, stamped by an admin's revoke or by a resend) returns no row, filtered in the function's own `WHERE` — the same treatment `auth_resolve_student_invitation` gives liveness, so a link already in a parent's inbox is dead as a property of the function rather than a check each caller must remember. `accepted_at`/`expires_at` stay in the return shape so the accept page can still say "already used" versus "expired". Unlike the staff equivalent, contact fields are read via a join to `guardians` — `guardian_invitations` stores no redundant copies (Decision: option b, new parallel table, not a reuse of `invitations`). | `token_hash`, `phone`, `created_at`. |
...
**Student password recovery (2026-08-31):** `auth_resolve_student_invitation`'s
return shape changed for the first time (adding `purpose`) — a DROP + CREATE of
the same function name, **not** a count-changing addition. Count stays at 22 and
the next review stays due at 23. The migration re-applies the same
`SET search_path = public, pg_temp`, `REVOKE ALL ... FROM PUBLIC` and
`GRANT EXECUTE ... TO app_user` the original CREATE carried, so
`security-definer-inventory.spec.ts` remains the conformance gate on all four
properties. Note the DROP + CREATE pattern is load-bearing here: a bare
`CREATE OR REPLACE` cannot change a function's `RETURNS TABLE` shape, and
dropping resets the grants, which is why they are restated rather than assumed.

**Guardian invitation revoke (2026-09-16):** `auth_resolve_guardian_invitation_by_token_hash`
gained `AND gi.revoked_at IS NULL` in its `WHERE` clause, alongside a new
nullable `guardian_invitations.revoked_at` column — the table's first revoke
path, added for admin resend/revoke. **Return shape unchanged**, so this was a
`CREATE OR REPLACE`, not a DROP + CREATE, and the existing REVOKE/GRANT carry
over; count stays at 22 and the next review stays due at 23. Proven at the SQL
level by `guardians.service.spec.ts` ("a revoked token stops resolving in SQL",
"resend kills the previous link in SQL"), which call the real function with the
...
- **The reverse case: an ESM-only package consumed from `apps/api`, which compiles to CommonJS.** `packages/config/tsconfig.node.json` sets `module: CommonJS` (NestJS needs it for decorator metadata), so in `apps/api` a plain `await import("pkg/x.mjs")` is downlevelled to `require()` and dies with `ERR_REQUIRE_ESM` at runtime. **Vitest does not reproduce this** — its SWC pipeline handles ESM natively, so the specs pass while the built API throws on the first request. The mirror image of the gap above, and the same lesson: tests passing is not proof of correct module resolution. Worse, the standard `new Function("s", "return import(s)")` bridge that fixes the compiled build *fails under Vitest* ("A dynamic import callback was not specified"), because the runner evaluates modules in a VM with no dynamic-import callback. Neither loader works in both places. `apps/api/src/modules/curriculum/parsing/document-parser.ts` carries the pattern that does: try the Function bridge (production), fall back to a plain dynamic import (Vitest). If you add an ESM-only dependency to `apps/api`, copy that shape — and verify by `require()`-ing the compiled `dist/` output, which is the only check that exercises the real path.
