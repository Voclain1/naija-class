...
// ---------------------------------------------------------------------------
// Phase 2 / Slice 8 isolation — subject_attendance_records (RLS coverage
// 22 → 23, Phase 2's final table). The subject-period row. Flat plain-FK (no
// enforced relations) → arbitrary plain-string ids; school_id is the only
// tenancy guard. Tagged via `note` so the fixture can find its rows.
// ---------------------------------------------------------------------------

describe("multi-tenant isolation — Phase 2 / Slice 8 (subject_attendance_records)", () => {
  const runId = Math.random().toString(36).slice(2, 8);
  let schoolA: { id: string };
  let schoolB: { id: string };
  let recA: { id: string };
  ...
  async function mark(schoolId: string, tag: string): Promise<{ id: string }> {
    return withTenant(schoolId, (db) =>
      db.subjectAttendanceRecord.create({
        data: {
          schoolId,
          studentId: `stu-${tag}-${runId}`,
          classArmId: `arm-${tag}-${runId}`,
  ...
  beforeAll(async () => {
    schoolA = await basePrisma.school.create({
      data: { name: "School P2S8A", slug: `rls-p2s8a-${runId}` },
      select: { id: true },
    });
    schoolB = await basePrisma.school.create({
      data: { name: "School P2S8B", slug: `rls-p2s8b-${runId}` },
      select: { id: true },
    });
    recA = await mark(schoolA.id, "a");
    recB = await mark(schoolB.id, "b");
  ...
  it("School A sees its own subject_attendance_record only", async () => {
    const rows = await withTenant(schoolA.id, (db) =>
      db.subjectAttendanceRecord.findMany({ where: { note: { contains: runId } } }),
    );
    const ids = rows.map((r) => r.id);
    expect(ids).toContain(recA.id);
    expect(ids).not.toContain(recB.id);
  ...
  it("School B sees its own subject_attendance_record only", async () => {
    const rows = await withTenant(schoolB.id, (db) =>
      db.subjectAttendanceRecord.findMany({ where: { note: { contains: runId } } }),
    );
    const ids = rows.map((r) => r.id);
  ...
  it("findUnique across tenants returns null", async () => {
    const leak = await withTenant(schoolA.id, (db) =>
      db.subjectAttendanceRecord.findUnique({ where: { id: recB.id } }),
    );
    expect(leak).toBeNull();
  });
  ...
  it("School B cannot update School A's subject attendance record", async () => {
    const result = await withTenant(schoolB.id, (db) =>
      db.subjectAttendanceRecord.updateMany({ where: { id: recA.id }, data: { status: "ABSENT" } }),
    );
    expect(result.count).toBe(0);
    const after = await withTenant(schoolA.id, (db) =>
      db.subjectAttendanceRecord.findUnique({ where: { id: recA.id }, select: { status: true } }),
    );
    expect(after?.status).toBe("PRESENT");
  });

  it("raw SQL with unset GUC returns zero rows from subject_attendance_records (FORCE RLS)", async () => {
    const rows = await basePrisma.$transaction(async (tx) => {
      return tx.$queryRaw<Array<{ id: string }>>`
        SELECT id FROM subject_attendance_records WHERE note LIKE ${"%" + runId + "%"}
      `;
    });
    expect(rows).toHaveLength(0);
  });
