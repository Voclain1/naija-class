...
// Phase 3 / Slice 13 cp1 — expense-category integration spec. Real DB + RLS.
// Covers: create (happy, name-conflict), findAll (active filter), findById,
// update (partial, name-change, conflict), delete (happy, has-expenses,
// not-found), cross-tenant isolation. Mirrors fee-category.service.spec.ts —
// same taxonomy shape, minus description/createdBy (not on this model).
...
describe("ExpenseCategoryService", () => {
  ...
  const svc = new ExpenseCategoryService();
  ...
  async function makeSchool(
    ...
    const signed = await auth.signupOwner(
      {
        schoolName: `Exp Cat ${suffix}`,
        schoolSlug: `exp-cat-${suffix}-${runId}`,
        ownerFirstName: "Ada",
        ownerLastName: "Owner",
        ownerEmail: `exp-cat-${suffix}-${runId}@example.test`,
  ...
  describe("create", () => {
    it("creates a category and returns dto with expenseCount=0", async () => {
      const { schoolId, ownerId } = await makeSchool("c1");
      const result = await svc.create(ctx(schoolId, ownerId), { name: "Utilities" }, reqCtx);
    ...
    it("rejects a duplicate name within the same school", async () => {
      ...
      await svc.create(ctx(schoolId, ownerId), { name: "Utilities" }, reqCtx);
      await expect(
        svc.create(ctx(schoolId, ownerId), { name: "Utilities" }, reqCtx),
    ...
    it("allows the same name in two different schools", async () => {
      ...
      await svc.create(ctx(a.schoolId, a.ownerId), { name: "Fuel" }, reqCtx);
      const result = await svc.create(ctx(b.schoolId, b.ownerId), { name: "Fuel" }, reqCtx);
    ...
    it("writes an audit log row for the create action", async () => {
      ...
      const created = await svc.create(ctx(schoolId, ownerId), { name: "Repairs" }, reqCtx);
      const log = await withTenant(schoolId, (db) =>
        db.auditLog.findFirst({
          where: { entityId: created.id, action: "expense-category.create" },
        }),
      );
  ...
  describe("findById", () => {
    ...
    it("throws NotFoundError when id belongs to another school (RLS isolation)", async () => {
      ...
      const created = await svc.create(ctx(a.schoolId, a.ownerId), { name: "Isolated Cat" }, reqCtx);
      await expect(
  ...
  describe("update", () => {
    it("updates the name", async () => {
      ...
      const created = await svc.create(ctx(schoolId, ownerId), { name: "Old Name" }, reqCtx);
      const updated = await svc.update(ctx(schoolId, ownerId), created.id, { name: "New Name" }, reqCtx);
      expect(updated.name).toBe("New Name");
    });

    it("deactivates a category", async () => {
      const { schoolId, ownerId } = await makeSchool("u2");
      const created = await svc.create(ctx(schoolId, ownerId), { name: "To Deactivate" }, reqCtx);
      const updated = await svc.update(ctx(schoolId, ownerId), created.id, { active: false }, reqCtx);
    ...
    it("throws NotFoundError for missing id", async () => {
      ...
      await expect(
        svc.update(ctx(schoolId, ownerId), "00000000-0000-0000-0000-000000000000", { name: "Ghost" }, reqCtx),
      ).rejects.toBeInstanceOf(NotFoundError);
    });
  });
  ...
  describe("delete", () => {
    it("deletes a category with no expenses", async () => {
      ...
      const created = await svc.create(ctx(schoolId, ownerId), { name: "Empty Cat" }, reqCtx);
    ...
    it("rejects deletion when category has expenses", async () => {
      const { schoolId, ownerId } = await makeSchool("d2");
      const authCtx = ctx(schoolId, ownerId);
      const cat = await svc.create(authCtx, { name: "Cat With Expenses" }, reqCtx);
      // Insert an expense directly to avoid importing ExpenseService here.
      await withTenant(schoolId, (db) =>
        db.expense.create({
          data: {
            ...
            categoryId: cat.id,
            amount: 50_000_00,
            incurredAt: new Date("2026-06-01"),
            recordedBy: ownerId,
          },
        }),
      );
      await expect(svc.delete(authCtx, cat.id, reqCtx)).rejects.toBeInstanceOf(ValidationError);
    });
    ...
    it("writes an audit log row for the delete action", async () => {
      ...
      await svc.delete(authCtx, cat.id, reqCtx);
      const log = await withTenant(schoolId, (db) =>
        db.auditLog.findFirst({ where: { entityId: cat.id, action: "expense-category.delete" } }),
      );
      expect(log).not.toBeNull();
    });
  });
