...
10. **`audit-coverage.spec.ts` extended for `finance.*`** — a "Phase 3 Finance audit coverage" describe block (mirroring the Phase 1/2 blocks) asserts audit rows for `fee-category.{create,delete}`, `fee-item.{create,delete}`, `discount-rule.{create,deactivate}`, `invoice.issue`, `payment.record`, `refund.create`, `expense.{create,delete}`, and `staff-bvn.{update,reveal}`. Every finance mutation writes its row.
