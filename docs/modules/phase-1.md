...
| 13 | **Phase 1 RBAC, audit coverage, RLS spec, E2E** — wire all new permissions into seed; update `admin` role on existing schools; verify every Phase 1 mutation writes its expected synchronous audit row (no BullMQ migration in this slice — that stays deferred); extend RLS isolation spec to all 15 new tables; add Playwright E2E for the CSV import wizard | Phase 1 closes here. All deliverables green. | 3 days |
