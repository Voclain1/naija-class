...
- [ ] Per-school AI enablement has no UI and no endpoint — `packages/db/scripts/disable-ai-per-school.ts` (2026-08-14) sets `School.aiEnabled = false` on every existing school so that flipping the platform-wide `AI_ENABLED` env var no longer switches AI on for everyone at once. Turning a school back ON is now the deliberate act it should be — but there is nothing to do it WITH: `platform-admin.controller.ts` exposes `login`, `GET schools`, `GET users`, `POST schools` and `PATCH schools/:schoolId/early-access`, and no AI toggle. Enabling the first pilot school today means a manual `UPDATE schools SET ai_enabled = true WHERE id = …`, which is exactly the kind of unaudited hand-write `PATCH …/early-access` exists to avoid for the marker beside it. Unblocked by: a `PATCH /platform-admin/schools/:schoolId/ai` mirroring the early-access endpoint (same guard, same audit row, same DTO shape) plus a toggle on the platform-admin school row. Trigger: before the first school is enabled, not after.

- [ ] `School.aiEnabled` still `@default(true)` for newly created schools — the backfill above is a point-in-time fix on the existing population. Every school created after it (signup or `POST /platform-admin/schools`) arrives with AI on, so the population drifts back open one school at a time and the backfill has to be re-run. Deliberately NOT changed by that script: the default-true is a considered decision documented in `schema.prisma` beside `parentSummaryEnabled`'s deliberately-opposite default-false, and reversing it is a product call, not a backfill's business. Whoever makes that call should decide it together with the item above — if enabling is a one-click platform-admin action, defaulting new schools to `false` costs nothing; while it's a manual SQL write, defaulting to `false` means every new school needs one.
