...
- [ ] **Wire Resend for real invitation email delivery — code shipped (Phase 4 Slice 6). PARTIALLY FIXED 2026-07-24: real API key is live and confirmed reaching Resend, but a second, independent blocker (unverified sending domain) means no guardian-invite email can actually be delivered yet. Not resolved.** Originally written pre-Phase-4 for staff invitations (which then just logged the accept URL for manual copy-paste); superseded by Slice 6's `EmailService`/`GuardiansService.deliverInvitation`, which does send real guardian-invite emails — but only if `RESEND_API_KEY` is actually configured. It never was: `docs/runbooks/neon-prod-setup.md` §5's `flyctl secrets set` template omitted the line entirely (now fixed — see that runbook's own new gotcha note next to the template), so the key was simply absent from `school-kit-api`'s Fly secrets the whole time, not a placeholder/wrong value like the `CORS_ORIGIN`/`WEB_BASE_URL`/`NEXT_PUBLIC_API_URL` incidents from the two days before this one.
  ...
  **Remaining blocker, needs Arinzechukwu directly**: verify the `schoolkit.ng` domain in Resend's dashboard (Domains section) by adding the SPF/DKIM DNS records Resend provides to wherever `schoolkit.ng`'s DNS is actually managed. Nothing further can be fixed via `flyctl` — this is account/DNS configuration only he (or whoever holds DNS access) can complete. Once verified, re-run the same `GuardiansService.invite()` end-to-end test (real app context, real guardian) to confirm delivery to a real, checkable inbox — not just that Resend accepts the send.
