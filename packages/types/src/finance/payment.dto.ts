...
export const recordManualPaymentSchema = z.object({
  ...
  // D37 — one key per payment FORM, generated by the client when the form
  // opens and reused on every retry of that same payment. A repeat returns the
  // first payment (replayed: true) and records nothing. Optional: the website
  // sends none and behaves exactly as before.
  idempotencyKey: z.string().uuid().optional(),
});
export type RecordManualPaymentInput = z.infer<typeof recordManualPaymentSchema>;

/**
 * POST /payments/manual's response. `replayed` is true when the request
 * carried an idempotency key already used for this school: nothing new was
 * recorded, and this is the payment the first request created.
 */
export interface ManualPaymentResultDto extends PaymentDto {
  replayed: boolean;
}
