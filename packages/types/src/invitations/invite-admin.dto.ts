...
// POST /users/invite — owner or admin invites a new admin, bursar, OR teacher
// to their school. Originally admin-only (Phase 0); Phase 3 slice 15 added
// the roleKey field for admin/bursar. Teacher was added later (2026-07-31,
// closing the deferred.md "single teacher invite" gap) once it was confirmed
// the generic accept flow (InvitationsService.accept) already resolves ANY
// seeded system role by key and never touches TeacherProfile — the same
// invite-only pattern the bulk CSV import (commit-teachers.row.ts) already
// used. TeacherProfile fields (staffNumber, specialty, etc.) are deliberately
// NOT captured here; the admin creates/edits that profile afterward via the
// existing /staff/[userId]/edit flow, identical to a CSV-imported teacher.
//
...
export const inviteAdminSchema = z
  .object({
    ...
    roleKey: z.enum(["admin", "bursar", "teacher"]).default("admin"),
  })
  .strict();
